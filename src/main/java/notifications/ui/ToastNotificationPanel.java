...
public class ToastNotificationPanel extends JPanel {
    ...
    public ToastNotificationPanel() { installDefault(); }
    ...
    private void installDefault() {
        ...
        putClientProperty(TOAST_ICON, labelIcon);
        putClientProperty(TOAST_COMPONENT, textPane);
    }
    ...
    public Icon getDefaultIcon() {
        switch (getKey()) {
            case "success" -> { return resizeIcon(check, size, size); }
            case "warning" -> { return resizeIcon(warning, size, size); }
            case "error" -> { return resizeIcon(error, size, size); }
            default -> { return resizeIcon(info, size, size); }
        }
    }
